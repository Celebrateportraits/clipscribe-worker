...
app.post("/process", async (req, res) => {
  ...
  try {
    ...
    // 5) videoReport — lightweight summary (full AI report happens in the Lovable app)
    const videoReport = [
      `# Source\n- URL: ${url}\n- Platform: ${platform}\n- Mode: ${mode}`,
      `# Media\n- Duration: ${durationSeconds}s\n- Resolution: ${resolution}\n- Frames extracted: ${frames.length} @ ${fps}fps`,
      transcript ? `# Transcript preview\n${transcript.slice(0, 600)}${transcript.length > 600 ? "…" : ""}` : `# Transcript\n(none — set GROQ_API_KEY to enable)`,
    ].join("\n\n");
